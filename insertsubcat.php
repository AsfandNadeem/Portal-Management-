...
<div class="row">
<form class="showform" action="insertall.php" method="GET">
  <div class="form-group">
    ...
    if($con)
    {
           ...
           echo "<select id='id' onclick='myFunction()' >";
           while($row=mysqli_fetch_assoc($result))
           {
            unset($id, $name);
            $id = $row['catId'];
            $name = $row['catName']; 
            echo "<option  name='incat' value={$id}>{$name}</option>";
           }
           echo "</select>";
    }
    ?>
   
    <input  name="subcatinsert" type="text" class="form-control" id="addsubcat" aria-describedby="emailHelp" placeholder="Enter SubCategory">
    <input name="incatsub" id="catselected"/>
  </div>
